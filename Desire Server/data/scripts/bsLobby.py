...
class Chooser(object):
    ...
    def __init__(self, vPos, player, lobby):
        ...
        bs.screenMessage(settings.welcomeMessage, color=((0+random.random()*1.0),(0+random.random()*1.0),(0+random.random()*1.0)), clients=[clID], transient=True)

        profile = self._player.get_account_id()

        #if we want to add but looks bad so i have it turned off
        if profile in gph.ownerHashes:
            bs.screenMessage('Owners has Joined the Match')

        if profile in gph.admin:
            bs.screenMessage('Admin has Joined the Match')
        
        if profile in gph.manager:
            bs.screenMessage('MSSSanager has Joined the Match')       

        if profile in sis.scanner:
            bs.screenMessage('Scanner has joined the match') 
     #   if profile in gph.member:
          #  bs.screenMessage('member has Joined the Match')
        ...
        if isinstance(msg, ChangeMessage):
            ...
            if msg.what == 'team':
                ...
                if forceTeamSwitch:
                    ...
                    bs.screenMessage("Unequal Teams!",color=(1, 0.1, 0.1),
