...
class PlayerSpaz_Smash(bs.PlayerSpaz):
    ...
    def oob_effect(self,m):
        ...
        if self.multiplyer > 1.25:
            ...
            radius = 7.5
        """
        if m.how != 'leftGame':bs.Blast(position=self.node.position, blastRadius=max(self.multiplyer * 5,7.5), blastType=self.bombType).autoRetain()
...
class SuperSmash(bs.TeamGameActivity):
    ...
    def onBegin(self):
        ...
        self._updateIcons()
        self.endchecktimer = bs.Timer(1000, bs.WeakCall(self._checkEndGame), repeat=False)
    ...
    def onPlayerJoin(self, player):
        if self.hasBegun():
            ...
            bs.screenMessage(bs.Lstr(resource='playerDelayedJoinText',subs=[('${PLAYER}',player.getName(full=True))]),color=player.color,
                            clients = [player.getInputDevice().getClientID()],
                            transient= True)
            # for score purposes, mark them as having died right as the game started
            player.gameData['lives'] = 0 if not self.timeLimitOnly else -999999
            player.gameData['icons'] = None
            if isinstance(self.getSession(),bs.FreeForAllSession):player.getTeam().gameData['survivalSeconds'] = 0
            return
        ...
        if isinstance(self.getSession(), bs.FreeForAllSession):
            ...
            for team in self.teams:
                if len(team.players) > 1:
                    ...
                    if player.gameData['icons'] is not None:
        ...
        if isinstance(m, bs.PlayerSpazDeathMessage):
            ...
            if player.gameData['lives'] == 0 and not self.timeLimitOnly:
                ...
                if all(teammate.gameData['lives'] == 0 for teammate in player.getTeam().players):
                    ...
                    # ffa clean unjoiner gamedata
                    #if player.gameData['lives'] == -1 and isinstance(self.getSession(),bs.FreeForAllSession):player.getTeam().gameData['survivalSeconds'] = 0
    ...
    def _checkEndGame(self):
            if isinstance(self.getSession(), bs.FreeForAllSession):
                ...
                if i <= 1:
                    self.endGame()
            elif isinstance(self.getSession(), bs.TeamsSession):
                for team in self.teams:
                    i=0
            ...
            for team in self.teams:
                for player in team.players:
                    ...
                    if 'survivalSeconds' in player.gameData:
                        ...
                        score = (curTime - self._startGameTime)//5000 + 1
                    #print player.gameData['lives']
                    #print isinstance(self.getSession(),bs.FreeForAllSession)
                    #if player.gameData['lives'] == -1 and isinstance(self.getSession(),bs.FreeForAllSession): score = 0
