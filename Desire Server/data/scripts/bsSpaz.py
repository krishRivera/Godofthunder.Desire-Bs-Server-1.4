...
class Spaz(bs.Actor):
        ...
        if isinstance(msg, bs.PickedUpMessage):
            ...
            if (msg.powerupType == 'tripleBombs'):
                ...
                if self.powerupsExpire:
                    ...
                    t = bs.getGameTime()
                    self.node.miniBillboard1StartTime = t
                    self.node.miniBillboard1EndTime = t+gPowerupWearOffTime-15000
    ...
    def dropBomb(self):
        ...
        if self.landMineCount > 0:
            ...
            self.setForceBombCount(self.forceBombCount-1)
            bombType = 'forceBomb'
        elif self.sleepPotionCount > 0:
            droppingBomb = False
            self.setSleepPotionCount(self.sleepPotionCount-1)
            bombType = 'sleepPotion'
    ...
    def setSleepPotionCount(self, count):
        """
        Set the number of land-mines this spaz is carrying.
        """
        self.sleepPotionCount = count
        if self.node.exists():
            if self.sleepPotionCount != 0:
                self.node.counterText = 'x' + str(self.sleepPotionCount)
                self.node.counterTexture = bs.Powerup.getFactory().texSleepPotionBombs
            else:
                self.node.counterText = ''
    ...
    def setEnderPearlCount(self,count):    
        self.enderPearlCount = count
        if self.node.exists():
            if self.enderPearlCount != 0:
                self.node.counterText = 'x' + str(self.enderPearlCount)
                self.node.counterTexture = bs.Powerup.getFactory().texEnderPearls
            else:
                self.node.counterText = ''
    ...
    def _getBombTypeTex(self):
        ...
        elif self.bombType == 'enderPearl': return bombFactory.texEnderPearls
    ...
    def _flashBillboard(self, tex):
        ...
        tex9 = bs.Powerup.getFactory().texSleepPotionBombs
    ...
    def _godWearOff(self):
        ...
        if self.node.exists():
            ...
            self.node.boxingGloves = 0
        if self.node.exists():
                setattr(self.node, 'hockey', False)
...
class PlayerSpaz(Spaz):
    ...
    def handleMessage(self, msg):
    	def sleep():
            if self.isAlive():
                if self._isSleeping:
                    self.node.handleMessage("knockout",3000)
                    pos = self.node.position
                    bsUtils.PopupText('Z',color=(1,2,6),scale=2,position=(pos[0],pos[1]-1.5,pos[2])).autoRetain()
                    bsUtils.animateArray(self.node,'color',3,{0:self.node.color,400:(2,0,2),800:self.node.color})
        ...
        def toxiced():
            if self.isAlive():
                if self._isToxic:
                    ...
                    bs.animateArray(self.node,'color',3,{0:(0,9,0),1000:self.node.color})
                    self.hitPoints -= 100
                    bs.emitBGDynamics(position=self.node.position,velocity=self.node.velocity,count=100,scale=0.3,spread=0.3,chunkType='slime');
                    if self.hitPoints <= 0:
                        self.node.handleMessage(bs.DieMessage())
        self._handleMessageSanityCheck()
        ...
        elif isinstance(msg,SleepMessage):
            if self._isSleeping: return
            self._isSleeping = True
            sleep()
            self._sleepTimer = bs.Timer(400,bs.Call(sleep),repeat=True)
            bs.gameTimer(3000,bs.Call(wakeUp))
        elif isinstance(msg,ToxicMessage):
            if self._isToxic: return
            self._isToxic = True
            self._toxicTimer = bs.Timer(1000,bs.Call(toxiced),repeat=True)
        elif isinstance(msg, bs.DieMessage):
