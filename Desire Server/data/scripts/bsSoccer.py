...
class SoccerGame(bs.TeamGameActivity):
    ...
    def onBegin(self):
      ...
      #bs.pushCall(bs.Call(bs.screenMessage,'KSR\'s Soccer Mod',color=(0,1,0),log=True)) 
    ...
    def spawnPlayer(self,player):
        ...
        def checkExistsSpaz():
            if spaz.node.exists():
                ...
                print '[SOCCER DEBUG] spazLight deleted'
    ...
    def _handleScore(self):
        ...
        bs.gameTimer(100,self._killPuck)

        light = bs.newNode('light',
                           attrs={'position': bs.getCollisionInfo('position'),
                                  'heightAttenuated':False,
                                  'color': (11,0.2,0)})
        bs.animate(light,'intensity',{0.5:0,500:10,500:4},loop=True, offset=20)
        bs.gameTimer(100,light.delete)

        self.cameraFlash(duration=3)
        
        self._updateScoreBoard()
        name = player.getName()
        bs.pushCall(bs.Call(bsInternal._chatMessage(name + 'Goal \U00026BD')))
